		...
		html.Div(
			...
			html.Div(
				html.H1(html.Class("text-3xl font-bold text-app-text dark:text-headline-dark flex items-center space-x-3"),
					...
					// Render GitHub icon if primary account is GitHub (GitHubID > 0), otherwise render GitLab icon if GitLabID > 0
					g.If(u.GitHubID > 0, html.A(
					...
					g.If(u.GitHubID == 0 && u.GitLabID > 0, html.A(
